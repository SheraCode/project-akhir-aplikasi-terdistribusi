...
class KelolaPengeluaran extends StatefulWidget {
  final int idPemasukan;

  const KelolaPengeluaran({Key? key, required this.idPemasukan}) : super(key: key);

  @override
  _KelolaPemasukanState createState() => _KelolaPemasukanState();
}

class _KelolaPemasukanState extends State<KelolaPengeluaran> {
  var height, width;
  File? _image;
  ...
  Future<void> fetchData() async {
    try {
      final Uri url = Uri.parse('http://192.168.21.80:2009/pengeluaran/${widget.idPemasukan}');
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _totalPemasukanController.text = data['jumlah_pengeluaran'].toString();
          _bentukPemasukanController.text = data['keterangan_pengeluaran'].toString();
          selectedKategori = data['id_kategori_pengeluaran'];
          _category = data['id_kategori_pengeluaran'];
          idPemasukan = data['id_pengeluaran'];
  ...
  Future<void> _updateKegiatan() async {
    try {
      ...
      var url = 'http://192.168.21.80:2009/pengeluaran';
      ...
      // Tambahkan data teks
      request.fields['jumlah_pengeluaran'] = _totalPemasukanController.text;
      request.fields['keterangan_pengeluaran'] = _bentukPemasukanController.text;
      request.fields['id_kategori_pengeluaran'] = _category.toString();
      request.fields['id_bank'] = _selectedBank.toString();
      request.fields['id_pengeluaran'] = idPemasukan.toString();
                          ...
                          child: Container(
                            ...
                            child: _image != null
                              ...
                              child: Image.file(
                                ...
                                : ClipOval(
                              child: Image.network(
                                'http://192.168.21.80:2009/pengeluaran/image/$idPemasukan',
                                height: 100,
                                width: 100,
                                fit: BoxFit.cover,
                              ),
